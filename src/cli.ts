...
const USAGE = `
    Usage
      $ awscognito export-users --user-pool-id <user-pool-id> [--format <JSON/CSV>] [--remove <one-two>]
...
async function exportUsersCli(cli: meow.Result): Promise<number> {
    ...
    const remove = (cli.flags.remove && cli.flags.remove.split("-")) || [];
    let format = "json";
    if (cli.flags.format) {
    ...
    if (format != "json" && format != "csv") {
        console.error(
            "[ERROR] Invalid export format: only JSON and CSV are supported!"
        );
        cli.showHelp();
    }
    ...
    try {
        return exportUsers(userpool, region, format, remove);
    } catch (err) {
        throw err;
    }
}
...
function importUsersCli(cli: meow.Result) {
    ...
    if (!userpool || !file) {
        console.error(
            "[ERROR] Missing arguments: --user-pool-id and --file are required!"
        );
        cli.showHelp();
    }
...
async function exportUsers(
    ...
    const describeUserPoolResponse = (await cognito
        .describeUserPool(params)
        .promise()) as DescribeUserPoolResponse;
    const filename: string = `${describeUserPoolResponse.UserPool.Name}.${format}`;
    let result: any[] = [];
    ...
    if (format === "json") {
        fs.writeFileSync(filename, JSON.stringify(cleanOutput(result, remove)));
    } else if (format === "csv") {
        const header = await getHeader(userpool, region);
        const text = csv(result, header);
...
function cleanOutput(input: any[], remove: string[]): any[] {
    return input.map((key) => omit(key, remove));
}
